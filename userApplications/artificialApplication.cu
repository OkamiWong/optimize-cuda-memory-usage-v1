...
void case_chainOfGemms(bool useGraph = true) {
  ...
  if (useGraph) {
    ...
    // Validate the annotation by checking whether the annotation
    // of the output of the first kernel matches with reality.
    ...
    size_t numRootNodes;
    checkCudaErrors(cuGraphGetRootNodes(graph, nullptr, &numRootNodes));
