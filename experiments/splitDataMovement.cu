...
void testPcieBandwidth(const std::vector<size_t> &sizes, bool useUnifiedMemory, bool noHeader) {
  ...
  for (auto size : sizes) {
    ...
    if (useUnifiedMemory) {
      ...
      for (int i = 0; i < SPLIT; i++) {
        checkCudaErrors(cudaMallocHost(&hostArray[i], size / SPLIT));
        memset(hostArray[i], 0, size / SPLIT);
      }

      int *deviceArray[SPLIT];
      for (int i = 0; i < SPLIT; i++) {
        checkCudaErrors(cudaMalloc(&deviceArray[i], size / SPLIT));
      }
      ...
      for (int i = 0; i < REPETITION; i++) {
        ...
        for (int j = 0; j < SPLIT; j++) {
          checkCudaErrors(cudaMemcpyAsync(deviceArray[j], hostArray[j], size / SPLIT, cudaMemcpyHostToDevice, stream));
        }
        clock.end(stream);
        checkCudaErrors(cudaStreamSynchronize(stream));
        minHostToDeviceTime = std::min(minHostToDeviceTime, clock.getTimeInSeconds());
        ...
        clock.start(stream);
        for (int j = 0; j < SPLIT; j++) {
          checkCudaErrors(cudaMemcpyAsync(hostArray[j], deviceArray[j], size / SPLIT, cudaMemcpyDeviceToHost, stream));
        }
        clock.end(stream);
        checkCudaErrors(cudaStreamSynchronize(stream));
        minDeviceToHostTime = std::min(minDeviceToHostTime, clock.getTimeInSeconds());
  ...
  if (useUnifiedMemory) {
    ...
    printDataOfTheSameKind("PCIe-HostToDevice-Split", sizes, hostToDeviceTimes);
    printDataOfTheSameKind("PCIe-DeviceToHost-Split", sizes, deviceToHostTimes);
  }
}
